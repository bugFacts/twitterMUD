...
PLAYERLIST = 'P:\python_twitter\scrips\DungeonMaster\savefiles\PLAYERLIST.txt'				
			
class Player:

	def __init__(self, userID=None, userName=None, reTime=None, 
				pos_x=None, pos_y=None, pos_z=None,
				xp=None, hp=None, maxhp=None,
...
class Player_Dict:
	...
	def load_from_file(self):
		...
		for i in fPlayerlist:
			...
			xp = i.split('|')[6].rstrip()
			hp = i.split('|')[7].rstrip()
			maxhp = i.split('|')[8].rstrip()
			
			char_name = i.split('|')[9].rstrip()
			char_class = i.split('|')[10].rstrip() 
			char_race = i.split('|')[11].rstrip()
			
			char_Helm=i.split('|')[12].rstrip()
			char_Chest=i.split('|')[13].rstrip()
			char_Hand=i.split('|')[14].rstrip()
			char_Ring=i.split('|')[15].rstrip()
			
			char_inv1 = i.split('|')[16].rstrip()
			char_inv2 = i.split('|')[17].rstrip()
			char_inv3 = i.split('|')[18].rstrip()
			
			char_inv = (char_inv1, char_inv2, char_inv3)
			
			enemy = i.split('|')[19].rstrip()
